# Mohammed Habeeb Mohsin Siddiqui

### 🚀 Data Science & Generative AI Enthusiast

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/habeeb-mohsin-b29225254)
[![GitHub](https://img.shields.io/badge/GitHub-Follow-black?style=for-the-badge&logo=github)](https://github.com/MHMSiddiqui)
[![Email](https://img.shields.io/badge/Email-Contact-red?style=for-the-badge&logo=gmail)](mailto:mhmsiddiquil7@gamil.com)

---

## 👋 About Me

I am a Computer Science undergraduate with a strong focus on **Data Science**, **Machine Learning**, and **Generative AI**.I have a solid foundation in Python, SQL, and modern AI frameworks, with hands-on experience in building LLM-powered applications and end-to-end machine learning solutions.I am passionate about translating real-world business problems into scalable, data-driven systems.

---

## 🛠️ Technical Skills

### 💻 Programming & Languages
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=postgresql&logoColor=white)

### 🤖 Generative AI & LLMs
* **Models:** OpenAI-GPT, Groq, Claude, LLAMA 2
* **Techniques:** Fine-tuning, RAG (Retrieval-Augmented Generation)
* **Frameworks:** LangChain, LlamaIndex, Hugging Face 

### 🧠 AI/ML Frameworks & Libraries
* TensorFlow, PyTorch, Keras, Scikit-learn 
* Pandas, NumPy, Matplotlib, Seaborn 

### 🗄️ Databases & Tools
* **Vector Databases:** Pinecone, Chroma, Cassandra.
* **Data Visualization:** Tableau, Power BI.
* **Tools:** Cursor AI, Anti Gravity, Claude Code.
* **IDEs & Version Control:** Jupyter Notebooks, Google Colab, VS Code, PyCharm, GitHub.


---

## 🎓 Education

**Bachelor of Technology in Computer Science and Engineering**
*Jawaharlal Nehru Technological University, Hyderabad* | *2022 - 2026*
* **CGPA:** 8.2/10.0 
* **Focus:** Data Science, Machine Learning, and AI Fundamentals.
* **Coursework:** Machine Learning Algorithms, Deep Learning, NLP, Databases, Statistical Analysis.

---

### 📫 Let's Connect!

I'm always open to discussing new projects, creative ideas, or opportunities in the AI space.

* **Email:** [mhmsiddiquil7@gamil.com](mailto:mhmsiddiquil7@gamil.com)
* **LinkedIn:** [Habeeb Mohsin](www.linkedin.com/in/habeeb-mohsin-b29225254) 
